...
def _action_reason(action: str) -> str:
    if action == "BUY":
        return "전 거래일 기준으로 장기 추세선 위이면서 단기선 위에 있어 오늘부터 보유 상태로 전환됩니다."
    if action == "SELL":
        return "전 거래일 기준으로 장기 추세선 아래이거나 단기선 아래로 내려가 오늘부터 현금 상태로 전환됩니다."
    if action == "HOLD":
        return "전 거래일 신호가 보유 조건(장기·단기 모두 종가 위)을 유지합니다."
    return "전 거래일 신호가 현금 대기 조건을 유지합니다."


def run_regime_ma_backtest(
    ...
    *,
    ...
    if len(price_data) < max_period + 2:
        raise ValueError("레짐·단기 이동평균과 익일 포지션을 계산하기에 가격 데이터가 부족합니다.")

    df = price_data.copy()
    df["ma_filter"] = df["close"].rolling(filter_period).mean()
    df["ma_signal"] = df["close"].rolling(signal_period).mean()
    df["ma5"] = df["close"].rolling(5).mean()
    df["ma20"] = df["close"].rolling(20).mean()
    df["ma60"] = df["close"].rolling(60).mean()
    df["ma120"] = df["close"].rolling(120).mean()
    ok = df["ma_filter"].notna() & df["ma_signal"].notna()
    df["signal"] = np.where(
        ok & (df["close"] > df["ma_filter"]) & (df["close"] > df["ma_signal"]),
        1,
        0,
    )
    ...
    price_points = [
        {
            ...
            "movingAverage": _clean_number(row.ma_signal),
    ...
    signals = [
        {
            ...
            "close": float(row.close),
            "movingAverage": _clean_number(row.ma_signal),
            "ma20": _clean_number(row.ma_signal),
    ...
    return {
        ...
        "strategyName": f"레짐 {filter_period}일 + 단기 {signal_period}일 MA 전략",
        "symbol": symbol,
